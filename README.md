...
6. (Optional) Further check your template file by letting `TEMPCHEK` write a "model input" file (parameter file), and then running the model with the generated file to make sure that values are being substitued into the template correctly. This is a bit of a chicken-egg problem, as `TEMPCHEK` must be able to find a `.par` file with the parameter values in it. While you could write the `.par` by hand it would be tedious and the file is actually generated by a PEST run. The `.par` file contains the best parameter set achieved so far. To run `TEMPCHEK` for this, provide a path to the template file and the path to the model input (parameter) file that should be written with parameter file. `TEMPCHEK` will look for a a `.par` file next to the template file.
